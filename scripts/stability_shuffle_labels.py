##Stability selection with shuffle
##Authors: Cristina Venturini & Yuxin Sun
# coding: utf-8

# This version of the script implement labels' permutation using random.shuffle() 

# We start with importing various functions/libraries
...
# We define a few functions we need (run_svm)
...
def run_svm(n, k, m, data, alpha_range, k_subsample):
    ...
    for counter in range(n):
       ...
       #subsample i samples 
        #samples = sample(range(n_sample), i)
        #samples_select.append(samples)
        #subsample_patients(label)
        
        data_n = data[:, idx]
        data_new, X_test, label_ord, y_test = train_test_split(data_n,label, test_size = 0.5, stratify=label)
        #print(label_ord)
        label_new = np.random.permutation(label_ord) ##this bit is shuffling the labels
        #print(label_new)
...
if __name__ == '__main__':
    ...
    data_tot = pd.read_csv('input_data.csv') 
    label = data_tot.Class.values #extract Class to use as label 
    label = np.squeeze(label.reshape(8,1)) #n of patients ###CHANGE HERE#####
    ...
    res_sorted.to_csv('out_shuffled.csv',sep=',') #save results as csv - this file is ready for SVM in R ###CHANGE HERE#####
